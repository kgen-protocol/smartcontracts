...
module KCashAdmin::kcash {
  ...
  struct RewardsBucket has key {
    ...
    #[view]
    public fun has_bucket_store(owner_addr: address):bool acquires RewardsBucket {
    ...
    #[view]
    public fun get_bucket_store(owner_addr: address): (u64, u64, u64) acquires BucketStore,RewardsBucket {
        let rewards_bucket = borrow_global<RewardsBucket>(@KCashAdmin);
...
fun create_bucket_storeV1(user: address) acquires BucketStore, RewardsBucket {
    ...
    fun create_bucket_store(user: &address) acquires BucketCore,RewardsBucket {
    ...
    fun ensure_bucket_store_exist(user: &address) acquires BucketCore , RewardsBucket{
    ...
    fun deposit_to_bucket(receiver: &address, reward1: u64, reward2: u64, reward3: u64) acquires BucketStore,RewardsBucket,BucketCore{
...
// transitioning to using a **contract store** to store user reward data due to high gas fee. 
    ...
    */ 
    ...
    fun withdraw_amount_from_bucket(owner: address, amount: u64) acquires RewardsBucket,BucketStore{
...
fun admin_transfer_reward3_to_user_bucket_internal(admin: &signer, user: &address, amount: u64, index: u8) acquires ManagedFungibleAsset, RewardsBucket, BucketStore {
    create_bucket_storeV1(signer::address_of(admin));
    create_bucket_storeV1(*user);
    let rewards_bucket = borrow_global_mut<RewardsBucket>(@KCashAdmin);
    // Scope-limited borrow to avoid overlap later
    {
    ...
    fun user_transfer_internal(from: &signer, to: &address, amount: &u64, index: u8) acquires RewardsBucket, ManagedFungibleAsset, BucketStore{
        ...
        let rewards_bucket = borrow_global_mut<RewardsBucket>(@KCashAdmin);
        let bucketSender = rewards_bucket.buckets.borrow_mut(&signer::address_of(from));
        ...
        if (index == 2) deposit_to_bucketV1(*to, 0, 0, *amount) else if(index == 1) deposit_to_bucketV1(*to, 0, *amount, 0) else deposit_to_bucketV1(*to, *amount, 0, 0);
    ...
    public entry fun mint(
        ...
        r2: u64, 
        r3: u64
    ) acquires ManagedFungibleAsset, AdminMinterRole ,RewardsBucket,BucketStore{
        ...
        deposit_to_bucketV1(to, r1, r2, r3);
    ...
    */
    public entry fun admin_transfer(admin: &signer, to: address, deductionFromSender: vector<u64>, additionToRecipient: vector<u64>)
        acquires ManagedFungibleAsset, BucketStore,AdminTransferRole,RewardsBucket
    {
    ...
    */
    ...
    public entry fun admin_transfer_reward3_to_user_bucket1(admin: &signer, to: address, amount: u64) acquires ManagedFungibleAsset, BucketStore,AdminTransferRole,RewardsBucket{
        assert!(verifyAdminTransfer(&signer::address_of(admin)), error::invalid_argument(EINVALID_ROLE));
        assert!(amount >= 0, error::invalid_argument(EINVALID_ARGUMENTS));
     ...
     */
    public entry fun admin_transfer_reward3_to_user_bucket1_bulk(admin: &signer, to_vec: vector<address>, amount_vec: vector<u64>) acquires ManagedFungibleAsset, BucketStore,AdminTransferRole,RewardsBucket{
     ...
     */
    public entry fun admin_transfer_reward3_to_user_bucket2(admin: &signer, to: address, amount: u64) acquires ManagedFungibleAsset, BucketStore,AdminTransferRole,RewardsBucket{
        assert!(verifyAdminTransfer(&signer::address_of(admin)), error::invalid_argument(EINVALID_ROLE));
        assert!(amount >= 0, error::invalid_argument(EINVALID_ARGUMENTS));

        // admin transfer internal
     ...
     * - The length of `to` array must be equal to the length of `amounts` array.
     */
    public entry fun admin_transfer_reward3_to_user_bucket2_bulk(admin: &signer, to_vec: vector<address>, amount_vec: vector<u64>) acquires ManagedFungibleAsset, BucketStore,AdminTransferRole,RewardsBucket{
        assert!(vector::length(&to_vec) == vector::length(&amount_vec), error::invalid_argument(EINVALID_ARGUMENTS_LENGTH));
        assert!(verifyAdminTransfer(&signer::address_of(admin)), error::invalid_argument(EINVALID_ROLE));
        let len = vector::length(&to_vec);
        assert!(len == vector::length(&amount_vec), error::invalid_argument(EINVALID_ARGUMENTS_LENGTH));
    ...
    acquires ManagedFungibleAsset, BucketStore, ManagedNonce, AdminSigner, AdminTransferRole,RewardsBucket {
        assert!(verifyAdminTransfer(&signer::address_of(admin)), error::invalid_argument(EINVALID_ROLE));
    ...
    public entry fun admin_transfer_with_signature_bulk(admin: &signer, to_vec: vector<address>, deductnFromSender1: vector<u64>, deductnFromSender2: vector<u64>, deductnFromSender3: vector<u64>, additnToRecipient1: vector<u64>, additnToRecipient2: vector<u64>, additnToRecipient3: vector<u64>, signature: vector<u8>)
        acquires ManagedFungibleAsset, BucketStore, RewardsBucket,ManagedNonce, AdminSigner, AdminTransferRole {
        assert!(verifyAdminTransfer(&signer::address_of(admin)), error::invalid_argument(EINVALID_ROLE));
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) 
        acquires ManagedFungibleAsset, BucketStore ,RewardsBucket {
    ...
    */
    public entry fun transfer_to_reward3(sender: &signer, to: address, bucket: vector<u64>) acquires ManagedFungibleAsset, RewardsBucket,BucketStore{
    ...
    */
    public entry fun transfer_to_reward3_bulk(sender: &signer, to_vec: vector<address>, bucket_vec: vector<vector<u64>>) acquires ManagedFungibleAsset,BucketStore,RewardsBucket{
        let len = vector::length(&to_vec);
    ...
    */
    public entry fun transfer_reward3_to_reward3 (from: &signer, to: address, amount: u64) acquires ManagedFungibleAsset,BucketStore,RewardsBucket{
        assert!(has_bucket_store(signer::address_of(from)), error::invalid_argument(EUSER_DO_NOT_HAVE_BUCKET_STORE));
    ...
    */
    ...
    acquires ManagedFungibleAsset, BucketStore,ManagedNonce, AdminSigner,RewardsBucket{
        assert!(amount >= 0, error::invalid_argument(EINVALID_ARGUMENTS));
    ...
    */
    ...
    acquires ManagedFungibleAsset,RewardsBucket,BucketStore, ManagedNonce, AdminSigner{
        let len = vector::length(&to_vec);
        assert!(len == vector::length(&amount_vec), error::invalid_argument(EINVALID_ARGUMENTS_LENGTH));
