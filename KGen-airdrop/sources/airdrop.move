module KGeNAdmin::airdrop {
    ...
    /// ChainID
    const CHAIN_ID: u64 = 2;  //For testnet

    /// Seed for creating a resource account
    const LAUNCHPAD_SEED: vector<u8> = b"KGeN Launchpad";
    ...
    // Event emitted when rewards are transferred with token address
    #[event]
    struct TransferV1 has drop, store {
        user: address, // Recipient of the reward
        amount: u64, // Amount transferred
        tokenAddress: address, //Address of token airdropped.
    }
    ...
    /// Manages treasury address for fee collection
    ...
    public fun get_treasury_address(): address acquires TreasuryStore {
        borrow_global<TreasuryStore>(@KGeNAdmin).treasury_address
    }
    ...
    fun init_module(admin: &signer) {
        ...
        // Initialize treasury store
        move_to(admin, TreasuryStore { treasury_address: signer::address_of(admin) });
    }
    ...
    public entry fun initialize_treasury(
        ...
        // Check if TreasuryStore already exists
        if (!exists<TreasuryStore>(@KGeNAdmin)) {
            move_to(admin_addr, TreasuryStore { treasury_address });
        };
    }
    ...
    ) acquires AdminStore, TreasuryStore {
        assert_admin(admin_addr);
        
        let treasury_store = borrow_global_mut<TreasuryStore>(@KGeNAdmin);
        treasury_store.treasury_address = new_treasury;
    }

    public entry fun claim_reward(
        claimer: &signer,
        object: address, // Assets (fund, token, coin, KGen) address, metadata address
        amount: u64,
        signature: vector<u8>
        ...
        let sender = &get_resource_account_sign(); // Ensure this address should be in whitelist at KGeN
        ...
        event::emit<TransferV1>(TransferV1 { user: signer::address_of(claimer), amount, tokenAddress: object });
        update_nonce(&signer::address_of(claimer), &object);
    }

    public entry fun claim_reward_v1(
        claimer: &signer,
        object: address, // Assets (fund, token, coin, KGen) address, metadata address
        amount: u64,
        feeAmount: u64,
        signature: vector<u8>
    ) acquires AdminStore, ManagedNonce, TreasuryStore {
        let nonce = ensure_nonce(claimer, &object);
        let message = SignedMessage {
            user: signer::address_of(claimer),
            metadata: object,
        ...
        let treasury = borrow_global<TreasuryStore>(@KGeNAdmin).treasury_address;
        let user_amount = amount - feeAmount;
        ...
        event::emit<TransferV1>(TransferV1 { 
            user: signer::address_of(claimer), 
            amount: user_amount,
            tokenAddress: object 
        });
    ...
    /// Debug function to get message hash for signature verification
    #[view]
    public fun debug_get_message_hash(
        user: address,
        metadata: address,
        amount: u64,
        nonce: u64
        ...
        let message_bytes = bcs::to_bytes<SignedMessage>(&message);
        hash::sha2_256(message_bytes)
    }
