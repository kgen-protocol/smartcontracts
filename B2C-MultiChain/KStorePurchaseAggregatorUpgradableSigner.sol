...
contract KStorePurchaseAggregatorUpgradableSigner is EIP712Upgradeable {
    string private constant SIGNING_DOMAIN = "KStorePurchaseAggregator";
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "Signature(address to,uint256 amount,uint256 nonce)"
                        ),
                        signature.to,
                        signature.amount,
                        signature.nonce
